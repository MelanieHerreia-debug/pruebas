...
const PROFESSORS = [
  {
    id: 'prof001',
    username: 'melanie.docente',
    password: 'profe2026',
    name: 'Dra. Melanie Ortiz',
...
const COURSES = [
  ...
  },
  {
    ...
    credits: 3,
    schedule: 'Lun/Mié/Vie 07:00–08:00',
  },
  {
    id: 'CS220',
...
const MOCK_STUDENTS = [
  {
    id: '1725384910',
    name: 'Carlos Mendoza',
    sessions: { CS301: 14, CS405: 10, CS512: 9, MAT202: 12, CS220: 13 },
  },
  {
    id: '1756294011',
    name: 'Sofía Ramírez',
    sessions: { CS301: 15, CS405: 14, CS512: 11, MAT202: 15, CS220: 15 },
  },
  {
    id: '1719384022',
    name: 'Diego Fuentes',
    sessions: { CS301: 8, CS405: 7, CS512: 6, MAT202: 9, CS220: 10 },
  },
  {
    id: '1728493011',
    name: 'Valentina Cruz',
    sessions: { CS301: 13, CS405: 12, CS512: 10, MAT202: 11, CS220: 14 },
  },
  {
    id: '1709483922',
    name: 'Andrés Paredes',
    sessions: { CS301: 5, CS405: 6, CS512: 4, MAT202: 6, CS220: 7 },
  },
  {
    id: '1759302944',
    name: 'Isabella Torres',
    sessions: { CS301: 15, CS405: 13, CS512: 12, MAT202: 14, CS220: 15 },
  },
  {
    id: '1720394811',
    name: 'Mateo Gómez',
    sessions: { CS301: 11, CS405: 10, CS512: 9, MAT202: 10, CS220: 12 },
  },
];
...
const Icon = {
  ...
  QR: () => (
    ...
    >
      ...
      <path d="M14 14h.01M18 14h.01M14 18h.01M18 18h.01M14 21h.01M21 14v.01M21 18v.01M21 21v.01" />
    </svg>
  ),
  Wifi: ({ color }) => (
    <svg
          ...
          <div style={{ padding: '24px' }}>
            ...
            <div style={s.fieldGroup}>
              ...
              <div style={s.selectWrapper}>
                ...
                >
                  <option value="falla_tecnica">
                    Falla de dispositivo móvil
                  </option>
                  <option value="sin_gps">
                    Error de geolocalización / GPS
                  </option>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
              <button style={{ ...s.ghostBtn, flex: 1 }} onClick={onClose}>
                Cancelar
              </button>
              <button
                style={{ ...s.warningBtn, flex: 2 }}
                onClick={handleSave}
                disabled={saving || !studentId.trim() || !studentName.trim()}
              >
                {saving ? (
  ...
  return (
    <div style={s.loginBg}>
      ...
      <div style={s.loginCard}>
        ...
        <p style={s.loginSubheading}>
          Ingresa al portal de control de asistencia
        </p>
    ...
    <div style={s.dashBg}>
      <header style={s.navbar}>
        ...
        <div style={s.navRight}>
          ...
          <button
            style={{ ...s.navBtn, color: '#f59e0b' }}
            onClick={() => setShowManual(true)}
          >
            <Icon.AlertTri />
            <span style={{ marginLeft: 6 }}>Asistencia Manual</span>
          </button>
          ...
          <div style={s.qrLayout}>
            <div style={s.qrControls}>
              ...
              <h1 style={s.dashTitle}>
                Código QR de
                <br />
                <span style={s.titleAccent}>Asistencia</span>
              </h1>
              <p style={s.dashSubtitle}>
                Elige la materia para habilitar el escaneo en tiempo real de los
                alumnos.
              </p>
              ...
              {activeCourse && (
                <div style={s.courseCard}>
                  <div style={s.courseCardRow}>
                    <span>Horario:</span>
                    <strong>{activeCourse.schedule}</strong>
                  </div>
                  <div style={s.courseCardRow}>
                    <span>Docente:</span>
              ...
              <button
                style={{
                  ...s.startBtn,
                  ...(classActive ? s.startBtnActive : {}),
                }}
                disabled={!selectedCourse}
                onClick={() => setClassActive(!classActive)}
              >
                {classActive
                  ? 'Detener Generación QR'
                  : 'Iniciar Clase / Generar QR'}
              </button>
            </div>
              ...
              {!classActive ? (
                <div style={s.qrPlaceholder}>
                  ...
                  <p style={{ color: '#475569', marginTop: 16 }}>
                    Inicia la clase para desplegar el código dinámico
                  </p>
                </div>
              ) : (
                <div style={s.qrActiveCard}>
                  ...
                  <div style={s.timerRow}>
                    <span style={{ fontSize: 13, color: '#94a3b8' }}>
                      Próxima rotación de token seguro en:{' '}
          ...
          <div style={s.reportPanel}>
            <div style={s.reportHeader}>
              <h2 style={s.reportTitle}>
                Dashboard de Control de{' '}
                <span style={s.titleAccent}>Asistencias</span>
              </h2>
              <div>
                <select
                ...
                <tbody>
                  {dashRows.map((row) => (
                    <tr key={row.id} style={s.tr}>
                      <td style={s.td}>
                        <span style={s.idBadge}>{row.id}</span>
                      </td>
                      <td style={s.td}>
                        <strong style={{ color: '#f8fafc' }}>{row.name}</strong>
                      </td>
                      <td style={s.td}>
                        {row.total} / {TOTAL_SESSIONS}
                      </td>
                      <td style={s.td}>
                        <div
                          style={{
                      ...
                      <td style={s.td}>
                        <span
                          style={{
                            ...s.statusPill,
                            color: row.pct >= 75 ? '#22c55e' : '#ef4444',
                          }}
                        >
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (pos) => {
            ...
            if (distancia <= RANGO_TOLERANCIA_M) {
              ...
              setStep('error');
              setErrorMsg(
                `Fuera de rango perimetral permitido (${Math.round(
                  distancia
                )} metros).`
              );
            }
          ...
          () => {
            setStep('error');
            setErrorMsg(
              'No se pudo acceder a la geolocalización. Activa los permisos de GPS.'
            );
          }
        );
      } else {
        setStep('error');
        setErrorMsg(
          'El navegador actual no soporta geolocalización de hardware.'
        );
      }
    }, 1500);
      ...
      <div style={s.studentCard}>
        {step === 'validating' && (
          <div style={{ padding: '40px 24px', textAlign: 'center' }}>
            ...
            <h3 style={{ color: '#fff', marginTop: 16 }}>
              Verificando Entorno de Red y GPS
            </h3>
            <p style={{ color: '#475569', fontSize: 13 }}>
              Validando coincidencia de IP y cercanía al campus...
            </p>
          </div>
        )}
        ...
        {step === 'form' && (
          <div style={s.formArea}>
            <div style={s.grantedBanner}>
              <span>
                ✔ Entorno Seguro Validado. Completa tus datos para finalizar.
              </span>
            </div>
            <h3 style={{ color: '#fff', margin: 0 }}>
              Formulario de Asistencia
            </h3>
            <p style={{ color: '#60a5fa', fontSize: 14, marginBottom: 20 }}>
              {courseObj?.name || courseId}
            </p>
            ...
            <form onSubmit={handleSubmit}>
              ...
              >
                <input
                  ...
                  style={{ transform: 'scale(1.2)', accentColor: '#3b82f6' }}
                />
                <label
                  htmlFor="checkAsis"
                  style={{ color: '#94a3b8', fontSize: 13, cursor: 'pointer' }}
                >
                  Registrar mi presencia en la clase de hoy
                </label>
              </div>

              <button type="submit" style={s.primaryBtn} disabled={!checked}>
        ...
        {step === 'success' && (
          <div style={s.successCard}>
            ...
            <p style={{ color: '#94a3b8', fontSize: 13, marginTop: 8 }}>
              Tu asistencia ha sido transmitida en tiempo real al panel del
              docente.
            </p>
          </div>
        )}
        ...
        {step === 'error' && (
          <div style={{ padding: '40px 24px', textAlign: 'center' }}>
            ...
            <h3 style={{ color: '#fff', marginTop: 12 }}>
              Error de Validación
            </h3>
  ...
  if (courseId && token) {
    return (
      <StudentScreen
        courseId={courseId}
        token={token}
        onRegisterSuccess={handleAttendanceSaved}
      />
    );
  }
